...
const oldHeroTitle = `<h1 className="text-4xl sm:text-[52px] lg:text-[64px] font-black leading-[1.05] tracking-tight text-gray-900 dark:text-white mb-4">
...
const newHeroTitle = `<h1 className="text-4xl sm:text-[52px] lg:text-[64px] font-black leading-[1.05] tracking-tight text-gray-900 dark:text-white mb-4">
...
if (content.includes(oldHeroTitle)) {
    ...
    console.log("Could not find hero title");
}

const oldFormButton = `Book Mechanic Now`;
content = content.replaceAll(oldFormButton, `Book a Mechanic Today`);
