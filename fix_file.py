...
# First let's remove the broken block and the extra `{/* HOW IT WORKS */}` lines.
# We will just replace everything from the first `{/* HOW IT WORKS */}` up to the `<section className="py-20 px-5 bg-gray-900`
import re

pattern = re.compile(r'\{\/\* HOW IT WORKS \*\/}.*?(?=<section className="py-20 px-5 bg-gray-900)', re.DOTALL)
            ...
            <div className="relative border-l-2 border-yellow-500/30 ml-4 md:ml-8 space-y-10 py-2">
              {[
                ...
                <div key={idx} className="relative pl-10 md:pl-12">
                  <div className="absolute -left-[21px] top-4 w-10 h-10 bg-yellow-500 text-gray-900 rounded-full flex items-center justify-center font-bold text-lg shadow-lg ring-8 ring-white dark:ring-[#1d1d1d]">
                    {idx + 1}
                  </div>
                  <div className="bg-gray-50 dark:bg-[#101010] p-6 md:p-8 rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.03)] dark:shadow-none border border-gray-100 dark:border-[#303030] hover:border-yellow-500/30 transition-colors group">
