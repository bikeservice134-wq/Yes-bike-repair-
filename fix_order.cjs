...
// Currently it might be: General, Jump Start, Engine Oil OR General, Engine Oil, Jump Start
// Let's just reconstruct the list
...
if (genServiceStart !== -1 && genServiceOilStart !== -1 && jumpStartStart !== -1 && gridEnd !== -1) {
  ...
  blockJump = blockJump.replace(/delay=\{[0-9]+\}/, 'delay={200}');
  blockOil = blockOil.replace(/delay=\{[0-9]+\}/, 'delay={300}');

  content = content.substring(0, positions[0].start) + blockGeneral + blockJump + blockOil + content.substring(gridEnd);
}

fs.writeFileSync('src/App.tsx', content, 'utf-8');
console.log("Fixed order explicitly.");
